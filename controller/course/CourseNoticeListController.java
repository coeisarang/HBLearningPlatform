 ...
 */
@WebServlet("/course/notice")
public class CourseNoticeListController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CourseNoticeListController() {
        super();
...
//		response.getWriter().append("Served at: ").append(request.getContextPath());
		...
		List<BoardDTO> list = dao.courseBoardList(courseIdx, "05", 10, pageNo);
		int count = dao.count(null, "05", null, courseIdx);
		dao.close();
		...
		request.setAttribute("url", "/course/notice?idx="+courseIdx+"&");
		
		request.setAttribute("courseInfo", courseInfo);
		request.setAttribute("list", list);
		request.getRequestDispatcher("/course/courseNoticeList.jsp").forward(request, response);
	}
